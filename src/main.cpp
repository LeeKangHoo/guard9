...
int cb(nfq_q_handle* q,nfgenmsg* message, nfq_data* packet, void* user_data){
    ...
    int result = tls_parse(payload_length-ip_length-tcp_length,payload+ip_length+tcp_length,&context->blocked_domains);
    if (result == 1){
...
int main(){
    ...
    std::string domain;
        while(std::getline(file,domain)){
            ...
            context.blocked_domains.insert(domain);
        }
        std::cout << context.blocked_domains.size() << " domains loaded" << std::endl;
