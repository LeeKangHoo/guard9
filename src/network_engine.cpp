...
int cb(nfq_q_handle* q,nfgenmsg* message, nfq_data* packet, void* user_data){
    ...
    if (payload_length < sizeof(ip_hdr)){
    ...
    //ip 기반 차단 기능
    bool detect_ip_result = detect_ip(&iphdr,context->config);
    if (detect_ip_result){
        //걸렸을때
    ...
    if (payload_length < ip_length + sizeof(tcp_hdr)){
        return nfq_set_verdict(q,packet_id,NF_ACCEPT,0,nullptr);
    }
    ...
    // blocked_domains.txt안에 있는 dns 탐지하고 판별하는 함수
    if (tcphdr.src_port == htons(443) || tcphdr.dst_port == htons(443)){
        int detect_sni_result = detect_sni(payload_length-ip_length-tcp_length,payload+ip_length+tcp_length,context->config);
        if (detect_sni_result == 1){
